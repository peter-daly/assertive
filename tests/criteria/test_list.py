...
def test_has_length_matches_pass():
    ...
    assert_that([1, 2, 3]).matches(has_length(is_gt(2)))
...
def test_has_length_does_not_match_pass():
    ...
    assert_that([1, 2, 3]).does_not_match(has_length(is_gt(4)))
...
def test_contains_matches_pass():
    ...
    assert_that([1, 2, 3]).matches(contains(is_gt(1)))
...
def test_contains_does_not_match_pass():
    ...
    assert_that([1, 2, 3]).does_not_match(contains(1, 2, is_gt(3)))
...
def test_contains_does_not_match_fail():
    with raises_exception(
        ...
        assert_that([1, 2, 3]).does_not_match(contains(is_gt(0)))


# contains_exactly
def test_contains_exactly_matches_pass():
    assert_that([1, 2, 3]).matches(contains_exactly(1, 2, 3))
    assert_that([1, 2, 3]).matches(contains_exactly(1, is_gt(1), is_gt(2)))
...
def test_contains_exactly_does_not_match_fail():
    with raises_exception(
        ...
        assert_that([1, 2, 3]).does_not_match(contains_exactly(is_gt(0), 2, 3))
